...
class TestAdvisorEmotionalModel:
    ...
    def test_emotional_contagion(self):
        ...
        initial_intensity = model.emotion_intensity
        
        # Test contagion from angry advisor with strong relationship
        model.emotional_contagion(EmotionalState.ANGRY, 0.9)
        
        # Should have some effect due to strong relationship - either emotion or intensity should change
        # The effect might be subtle but should be measurable
        emotion_changed = model.current_emotion != initial_emotion
        intensity_changed = abs(model.emotion_intensity - initial_intensity) > 0.01
        
        assert emotion_changed or intensity_changed, f"Expected emotional contagion effect, but emotion stayed {model.current_emotion} and intensity stayed {model.emotion_intensity}"
