...
class TestVLLMProvider:
    ...
    def test_validate_messages_valid(self):
        ...
        config = LLMConfig(provider=LLMProvider.VLLM, model="test-model")
        provider = VLLMProvider(config)
...
class TestLLMManager:
    ...
    def test_manager_initialization(self):
        ...
        primary_config = LLMConfig(provider=LLMProvider.VLLM, model="test-model")
        fallback_configs = [
            LLMConfig(provider=LLMProvider.OPENAI, model="gpt-3.5-turbo", api_key="test")
        ]
    ...
    async def test_generate_fallback(self, mock_openai):
        ...
        primary_config = LLMConfig(provider=LLMProvider.VLLM, model="test-model")
        fallback_config = LLMConfig(provider=LLMProvider.OPENAI, model="gpt-3.5-turbo", api_key="test")
